...
/*
 * Reference for this code is: https://github.com/Xety/Cake3-Upload
 * Find Edits with [UPDATED]
 */
...
class UploadBehavior extends Behavior {
    ...
    /**
     * Save path in db like uploads/banners/f87d563f8ad357a5e31c570a4a7a24fc.jpg
     *
     * @var bool
     */
    protected $savePath = false; // used to remove filepath from filename [UPDATED]
    ...
    public function beforeSave(Event $event, Entity $entity) {
        ...
        foreach ($config['fields'] as $field => $fieldOption) {
            ...
            $filenameOrig = str_replace(".{$extension}", '', $file['name']); // pass original filename [UPDATED]
            $uploadPath = $this->_getUploadPath($entity, $fieldOption['path'], $extension, $filenameOrig);
            if (!$uploadPath) {
            ...
            if ($this->_moveFile($entity, $file['tmp_name'], $uploadPath, $field, $fieldOption)) {
                if (!$this->savePath) { // remove filepath with filename [UPDATED]
                    $fileNamesArr = explode(DS, $uploadPath);
                    $nameToSave = $this->_prefix . end($fileNamesArr);
                } else {
                    $nameToSave = $this->_prefix . $uploadPath;
                }
                $entity->set($field, $nameToSave);
            }
    ...
    protected function _deleteOldUpload(Entity $entity, $field = false, $newFile = false, array $options = []) {
        ...
        if (!$this->savePath) { // attach filepath with filename [UPDATED]
            $oldPath = $this->_getOldPath($entity, $options['path'], $entity->$field);
            //$origPathArr = explode(':', $options['path']); //path is before the :
            $entity->$field = $oldPath;
        }
     ...
     *      :name   : Modified name of the file
     *
     ...
     * @return bool|string
     */
    protected function _getUploadPath(Entity $entity, $path = false, $extension = false, $originalName = '') {
        ...
        $identifiers = [
            ':id' => $entity->id,
            ':name' => uniqid() . Inflector::slug($originalName), // attach filepath with filename [UPDATED]
            ':custom_name' => rand().'-'.date('d').'-'.date('m').'-'.date('Y').'-'.date('H').'-'.date('i').'-'.date('s'),
    ...
    protected function _getOldPath(Entity $entity, $path = false, $originalName = '') {
        if ($path === false) {
            return false;
        }
        $path = trim($path, DS);
        $identifiers = [
            ':id' => $entity->id,
            ':name' => $originalName, // attach filepath with filename [UPDATED]
        ];
        return strtr($path, $identifiers);
    }
