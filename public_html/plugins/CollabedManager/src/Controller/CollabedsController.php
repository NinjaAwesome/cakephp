...
class CollabedsController extends AppController
{
    ...
    public function index()
    {
        ...
        $query->select(['total'=> $query->func()->count('CollabedLikes.collabed_id')])
            ->autoFields(true)
            ->contain(['CollabedLikes','Artistsone','Artiststwo', 'UsersSoc'])
            ...
            if($user)
            {
                if($this->request->getQuery('mycollab'))
                {
                    if($this->request->getQuery('mycollab') == 'true')
                    {
                        
                            $query->where(['Collabeds.user_id' => $user['id']]);
                    }
                }
           } 
    ...
    public function ajaxCollabe(){
        ...
        if ($this->request->is('ajax')) {
            ...
            if($this->request->getQuery('search_keyword')) {
                ...
                if($if_space)
                {
                    ...
                    if($artist_qury->count() == 0)
                    {

                        $pos = strpos($search, " feat ");
                        ...
                        if ($pos === false) {
                            ...
                            foreach($search_arr as $item)
                            {
                                    $artist_qury = $this->Artists->find('active')
                                        ->select(['Artists.id'])
                                        ->where(['Artists.name LIKE'=> trim($item).'%']);
                                    $group_qury = $this->Artists->Groups->find()
                                        ->select(['Groups.artist_id'])
                                        ->where(['Groups.name LIKE'=> trim($item).'%','Artists.status' => 1])
                        ...
                        {
                            $search_arr = explode(' feat ', $search);
                            $feat = true;
        ...
        if(count($final_artis) > 0){
            //->find('artistactive')
            $query = $this->Collabeds->find('active');
//            $query->find('filter', $this->request->getQuery());
            $query->select(['total'=> $query->func()->count('CollabedLikes.collabed_id')])
                ->autoFields(true);  
                if(!empty($this->request->getQuery('search_keyword'))){

                    $final_artis = array_unique($final_artis);

                    if($feat)
                    {
                        if(count($final_artis) > 2)
                        {
                            $query->where(['Collabeds.artist_1' => $final_artis[0], 'Collabeds.artist_2 ' => $final_artis[1]]);
                            $query->orWhere(['Collabeds.artist_2' => $final_artis[0], 'Collabeds.artist_1' => $final_artis[1]]);

                            for($i = 1; $i < count($final_artis); $i++)
                            {
                                if(isset($final_artis[$i+1]))
                                {
                                    $query->orWhere(['Collabeds.artist_2' => $final_artis[$i], 'Collabeds.artist_1' => $final_artis[$i+1]]);
                                    $query->orWhere(['Collabeds.artist_2' => $final_artis[$i+1], 'Collabeds.artist_1' => $final_artis[$i]]);

                                }
                            }
                        }
                        elseif(count($final_artis) == 2)
                        {
                            $query->where(['Collabeds.artist_1' => $final_artis[0], 'Collabeds.artist_2' => $final_artis[1]]);
                            $query->OrWhere(['Collabeds.artist_1' => $final_artis[1], 'Collabeds.artist_2' => $final_artis[0] ]);

                        }
                        else
                        {
                            $query->where(['Collabeds.artist_1 IN ' => 0]);
                            $query->orWhere(['Collabeds.artist_2 IN ' => 0]);
                        }
                ...
                $query->contain(['CollabedLikes','Artistsone','Artiststwo', 'UsersSoc'])
                ->leftJoinWith('CollabedLikes')
                ->group(['Collabeds.id']);
                if($this->request->query('like') == 'desc'){
                    $query->order(['total' => 'desc']);
        ...
        else
        {
            ...
            if ($this->request->is('post')) {
                ...
                if ($this->Collabeds->save($collabed)) {
                    $this->Flash->success(__('Thanks! This collab has been saved.'));

                    return $this->redirect(['action' => 'index']);
                }
                $this->Flash->error(__('This collab could not be saved. Please, try again.'));
            }
        }
            $finalData = [];
            $banner = $this->Banner->get(1);
            ...
            $this->set('createheader', true);
            $this->set(compact('collabed', 'banner','artists'));
     
        
        $this->set('user', $this->Auth->user() ? $this->Auth->user() : []);   
        $this->set('title_new', "Create"); 
    }
    ...
    public function create()
    {
        ...
        $requestData['url'] =  strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $article_one->name .'-'.$article_two->name)));
...
//        pr($itemInfoArr);die;
        ...
        $requestData['banner_id'] =  $banner_id;
        $requestData['image'] =  $collabe;
        $requestData['url'] =  strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $artistone .'-feat-'.$artisttwo)));
        $requestData['user_id'] =  $this->Auth->user('id');
        ...
        if ($this->Collabeds->save($collabed)) {
            if(!$two_new || !$one_new){
                $this->Flash->success(__('Thanks! We will add this collab after we approve this artist.'));
            }else{
                $this->Flash->success(__('Thanks! This collab has been saved.'));
            }

            echo $collabed->id;exit;

            // return true;
            // return $this->redirect(['action' => 'add', $collabed->id]);
        }
        echo 0;exit;
    }
    ...
    public function getCollabedsByName()
    {
        ...
        if ($this->request->is('ajax')) {
            ...
            if($artistone_name == '')
            {
                ...
                $query->andwhere(['Collabeds.status' => 1]);
                $query->limit(1);
                $query->order('rand()');
                $query->select(['total'=> $query->func()->count('CollabedLikes.collabed_id')])
                    ->autoFields(true)
            ...
            elseif(strlen ($artistone_name) == 1)
            {


                $artistsOne = $this->Artists->find('list',[
                    'keyField' => 'id'
                ])->where(['Artists.name LIKE ' => strtolower($artistone_name) . '%'])
                    ->toArray();

                if(count($artistsOne) == 0)
                {
                    $query = $this->Collabeds->find();//->find('active');
                    $query->where(['Collabeds.banner_id' => $banner_id]);
                    $query->andwhere(['Collabeds.status' => 1]);
                ...
                else
                {
                    ...
                    $query->where(['Collabeds.banner_id' => $banner_id]);
                    $query->andwhere(['Collabeds.status' => 1]);
                    $query->andwhere(['OR'=>['Collabeds.artist_1 IN ' => array_keys($artistsOne),'Collabeds.artist_2 IN ' => array_keys($artistsOne)]]);
                    $query->limit(1);
                    $query->order('rand()');
                    $query->select(['total'=> $query->func()->count('CollabedLikes.collabed_id')])
                        ->autoFields(true)
            ...
            elseif($artistone_name != $artisttwo_name){
...
//            pr($artistsTwo);die;
            //pr($banner);die;
           //pr($collabeds);die;
            $this->set(compact('banner','banner_id' ,'collabeds','artistone_name','artisttwo_name','artistsOne','artistsTwo','artisBname','artisAname', 'type'));
            $this->set('user', $this->Auth->user() ? $this->Auth->user() : []);
        }
    }
